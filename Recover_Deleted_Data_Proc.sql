...
AS (SELECT [RowLog Contents 0] AS [RowLogContents],
           ...
           (CASE
                WHEN SUBSTRING([RowLog Contents 0], 1, 1) IN ( 0x10, 0x30, 0x70 ) THEN
                    CONVERT(
                               INT,
                               CONVERT(
                                          BINARY(2),
...
IF
(
    SELECT COUNT(*)FROM @DeletedRecords
) = 0
BEGIN
...
SELECT [Row ID],
       ...
       (CASE
            WHEN [system_type_id] IN ( 35, 34, 99 ) THEN
                16
            ELSE
                24
        END
                                ...
                                SELECT TOP 1
                                       ISNULL(SUM(   CASE
                                                         WHEN C.leaf_offset > 1
                                                              AND C.leaf_bit_position = 0 THEN
                                                             max_length
                                                         ELSE
                                                             0
                                                     END
                             ...
                             (
                                 ...
                                 WHERE B.[Page ID] = C.[Page ID]
                                       AND SUBSTRING(
                                                        ...
                                                        (CHARINDEX('Offset', B.[ParentObject])
                                                         - (CHARINDEX('Slot', B.[ParentObject]) + 4)
                                                        )
                                                    ) = SUBSTRING(
                                                                     C.[ParentObject],
                                                                     CHARINDEX('Slot', C.[ParentObject]) + 4,
                                                                     (CHARINDEX('Offset', C.[ParentObject])
                                                                      - (CHARINDEX('Slot', C.[ParentObject]) + 4)
                                                                     )
                                                                 )
...
SET [RowLog Len] = CONVERT(
                              VARBINARY(8000),
                              REVERSE(CAST('' AS XML).value(
...
SET B.[RowLog Contents 0] = (CASE
                                 WHEN A.[RowLog Contents 0] IS NOT NULL
                                      AND C.[RowLog Contents 0] IS NOT NULL THEN
                                     A.[RowLog Contents 0] + C.[RowLog Contents 0]
                                 WHEN A.[RowLog Contents 0] IS NULL
                                      AND C.[RowLog Contents 0] IS NOT NULL THEN
                                     C.[RowLog Contents 0]
                                 WHEN A.[RowLog Contents 0] IS NOT NULL
                                      AND C.[RowLog Contents 0] IS NULL THEN
                                     A.[RowLog Contents 0]
                             END
                            ),
...
FROM @ModifiedRawData B
    LEFT JOIN @ModifiedRawData A
        ON A.[PAGE IDS] = CONVERT(INT, CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING(B.[RowLog Contents 0], 15 + 14, 2))))
           AND A.[FILE IDS] = CONVERT(
                                         INT,
                                         CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING(B.[RowLog Contents 0], 19 + 14, 2)))
                                     )
           AND A.[Link ID] = B.[Link ID]
    LEFT JOIN @ModifiedRawData C
        ON C.[PAGE IDS] = CONVERT(INT, CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING(B.[RowLog Contents 0], 27 + 14, 2))))
           AND C.[FILE IDS] = CONVERT(
                                         INT,
                                         CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING(B.[RowLog Contents 0], 31 + 14, 2)))
                                     )
                               ...
                               WHEN [RowLog Len] < 8000 THEN
                                   SUBSTRING(
                                                ...
                                                CONVERT(
                                                           ...
                                                           CONVERT(
                                                                      VARBINARY(MAX),
                                                                      REVERSE(SUBSTRING([RowLog Contents 0], 15, 6))
                                                                  )
                                                       )
                                            )
                           END
...
FROM @ColumnNameAndData A
    INNER JOIN @ModifiedRawData B
        ON CONVERT(INT, CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING([hex_Value], 17, 4)))) = [PAGE IDS]
...
SET [hex_Value] = (CASE
                       WHEN B.[RowLog Contents 0] IS NOT NULL
                            AND C.[RowLog Contents 0] IS NOT NULL THEN
                           B.[RowLog Contents 0] + C.[RowLog Contents 0]
...
FROM @ColumnNameAndData A
    LEFT JOIN @ModifiedRawData B
        ON CONVERT(INT, CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING([hex_Value], 5, 4)))) = B.[PAGE IDS]
           AND B.[Link ID] = 0
    LEFT JOIN @ModifiedRawData C
        ON CONVERT(INT, CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING([hex_Value], 17, 4)))) = C.[PAGE IDS]
           AND C.[Link ID] = 0
WHERE [system_type_id] IN ( 99, 167, 175, 231, 239, 241, 165, 98 )
      AND
      (
          B.[RowLog Contents 0] IS NOT NULL
          OR C.[RowLog Contents 0] IS NOT NULL
      );
...
FROM @ColumnNameAndData A
    INNER JOIN @ModifiedRawData B
        ON CONVERT(INT, CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING([hex_Value], 9, 4)))) = [PAGE IDS]
...
FROM @ColumnNameAndData A
    INNER JOIN @ModifiedRawData B
        ON CONVERT(INT, CONVERT(VARBINARY(MAX), REVERSE(SUBSTRING([hex_Value], 15, 4)))) = [PAGE IDS]
WHERE [system_type_id] IN ( 35, 34, 99 )
      AND [Link ID] = 0;
...
SELECT NAME,
       ...
       (CASE
        ...
        END
           ...
           WHEN system_type_id = 98 THEN
       (CASE
            WHEN CONVERT(INT, SUBSTRING(hex_Value, 1, 1)) = 56 THEN
                CONVERT(
                           VARCHAR(MAX),
            ...
            WHEN CONVERT(INT, SUBSTRING(hex_Value, 1, 1)) = 108 THEN
                CONVERT(
                           ...
                           CONVERT(
                                      NUMERIC(38, 20),
                                      CONVERT(VARBINARY(1), SUBSTRING(hex_Value, 3, 1))
                                      + CONVERT(VARBINARY(1), SUBSTRING(hex_Value, 4, 1)) + CONVERT(VARBINARY(1), 0)
                                      + SUBSTRING(hex_Value, 5, LEN(hex_Value))
                                  )
                       )                                                                        --- NUMERIC
            WHEN CONVERT(INT, SUBSTRING(hex_Value, 1, 1)) = 167 THEN
                LTRIM(RTRIM(CONVERT(VARCHAR(MAX), SUBSTRING(hex_Value, 9, LEN(hex_Value)))))    --VARCHAR,CHAR
            WHEN CONVERT(INT, SUBSTRING(hex_Value, 1, 1)) = 36 THEN
                CONVERT(VARCHAR(MAX), CONVERT(UNIQUEIDENTIFIER, SUBSTRING((hex_Value), 3, 20))) --UNIQUEIDENTIFIER
            ...
            WHEN CONVERT(INT, SUBSTRING(hex_Value, 1, 1)) = 165 THEN
                '0x'
                + SUBSTRING(
                               (CASE
                                    WHEN CHARINDEX(
...
SET @SQL
    = 'SELECT ' + @FieldName + ' FROM #temp_Data PIVOT (Min([FieldValue]) FOR FieldName IN (' + @FieldName
      + ')) AS pvt';
