ALTER PROCEDURE dbo.Recover_Modified_Data_Proc
...
BEGIN
    ...
    SET @hex_pageid
        = '0x' + SUBSTRING(@ConsolidatedPageID, CHARINDEX(':', @ConsolidatedPageID) + 1, LEN(@ConsolidatedPageID)); ---Seperate the page ID
...
FROM sys.fn_dblog(NULL, NULL) A
    INNER JOIN @pagedata B
        ON A.[Page ID] = B.[Page ID]
...
BEGIN
    ...
    BEGIN
        ...
        */
        ...
        SET @RowLogContents3_Var
            = CAST('' AS XML).value('xs:hexBinary(sql:variable("@RowLogContents3") )', 'varchar(max)');
        ...
        SELECT @RowLogData_Hex
            = CAST('' AS XML).value('xs:hexBinary( substring(sql:variable("@RowLogData_Var"),0) )', 'varbinary(max)')
        FROM
        (
        ...
        */
        ...
        SET @FixedLengthIncrease = (CASE
                                        WHEN (LEN(@FixedOldValues) % 4) = 0 THEN
                                            1
                                        ELSE
        (4 - (LEN(@FixedOldValues) % 4))
                                    END
                                   );
        ...
        SET @VariableOldValues
            = SUBSTRING(
                           @RowLogData_Hex,
                           @OldFixedLengthStartPosition + @FixedLength_Loc + @FixedLengthIncrease + @FixedLength_Loc
                           + (@FixedLengthIncrease),
                           @VariableLength_Loc
                       );
        SET @VariableLengthIncrease = (CASE
                                           WHEN (LEN(@VariableOldValues) % 4) = 0 THEN
                                               1
                                           ELSE
        (4 - (LEN(@VariableOldValues) % 4)) + 1
                                       END
                                      );
        SET @VariableOldValues = (CASE
                                      WHEN @VariableLength_Loc = 1 THEN
                                          @VariableOldValues + 0x00
                                      ELSE
                                          @VariableOldValues
                                  END
                                 );
        ...
        SET @VariableNewValues
            = SUBSTRING(
                           SUBSTRING(
                                        @RowLogData_Hex,
                                        @OldFixedLengthStartPosition + @FixedLength_Loc + @FixedLengthIncrease
                                        + @FixedLength_Loc + (@FixedLengthIncrease - 1) + @VariableLength_Loc
                                        + @VariableLengthIncrease,
        ...
        */
        ...
        SELECT TOP 1
               @VariableNewValues
                   = CASE
                         WHEN CHARINDEX(SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) + 1), [RowLog Contents 0]) <> 0 THEN
                             SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) + 1)
                         WHEN CHARINDEX(SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues)), [RowLog Contents 0]) <> 0 THEN
                             SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues))
                         WHEN CHARINDEX(SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) - 1), [RowLog Contents 0]) <> 0 THEN
                             SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) - 1) --3 --Substring(@VariableNewValues,0,Len(@VariableNewValues)-1)
                         WHEN CHARINDEX(SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) - 2), [RowLog Contents 0]) <> 0 THEN
                             SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) - 2)
                         WHEN CHARINDEX(SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) - 3), [RowLog Contents 0]) <> 0 THEN
                             SUBSTRING(@VariableNewValues, 0, LEN(@VariableNewValues) - 3) --5--Substring(@VariableNewValues,0,Len(@VariableNewValues)-3)
                     END
        FROM @ModifiedRawData
        WHERE [Slot ID] = @Slotid
        ORDER BY [ID] DESC;
        ...
        SELECT TOP 1
               ...
               + CAST(REPLACE(
                                 SUBSTRING([RowLog Contents 0], @VariableLength_Offset + 1, LEN(@VariableNewValues)),
                                 @VariableNewValues,
                                 @VariableOldValues
                             ) AS VARBINARY)
               + SUBSTRING(
                              [RowLog Contents 0],
                              @VariableLength_Offset + LEN(@VariableNewValues) + 1,
...
AS (SELECT [RowLog Contents 0] AS [RowLogContents],
           ...
           (CASE
                WHEN SUBSTRING([RowLog Contents 0], 1, 1) IN ( 0x30, 0x70 ) THEN
                    CONVERT(
                               INT,
                               CONVERT(
                                          BINARY(2),
           ...
           (CASE
                WHEN SUBSTRING([RowLog Contents 0], 1, 1) IN ( 0x30, 0x70 ) THEN
                    SUBSTRING(
                                 ...
                                 CONVERT(
                                            SMALLINT,
                                            CONVERT(BINARY(2), REVERSE(SUBSTRING([RowLog Contents 0], 2 + 1, 2)))
                                        ) + 3
                                 + CONVERT(
                                              INT,
                                              CEILING(CONVERT(
       ...
       [NullBitMap] = (REPLACE(
                                  ...
                                  (
                                      SELECT ','
                                             + (CASE
                                                    WHEN [ID] = 0 THEN
                                                        CONVERT(NVARCHAR(1), (SUBSTRING(NullBytes, n, 1) % 2))
                                                    ELSE
                                                        CONVERT(
                                                                   NVARCHAR(1),
                                                                   ((SUBSTRING(NullBytes, n, 1) / [Bitvalue]) % 2)
                                                               )
                                                END
                                               ) --as [nullBitMap]
                                      FROM N4 AS Nums
                                          JOIN RowData AS C
                                              ON n <= NullBitMapLength
                                          CROSS JOIN @bitTable
...
AS (
   ...
   SELECT A.[ID],
          ...
          (CASE
               WHEN leaf_offset < 1
                    AND SUBSTRING([NullBitMap], cols.leaf_null_bit, 1) = 0 THEN
                   CONVERT(
                              SMALLINT,
          ...
          (CASE
               WHEN leaf_offset < 1
                   ...
                   CONVERT(
                              ...
                              CONVERT(
                                         BINARY(2),
                                         REVERSE(SUBSTRING([ColumnOffsetArray], (2 * (leaf_offset * -1)) - 1, 2))
                                     )
                          )
                   - ISNULL(
                               NULLIF(
          ...
          CASE
              WHEN SUBSTRING([NullBitMap], cols.leaf_null_bit, 1) = 1 THEN
                  NULL
              ELSE
                  SUBSTRING(
                               RowLogContents,
...
SELECT name,
       CASE
           ...
           WHEN system_type_id IN ( 167, 175 ) THEN
               LTRIM(RTRIM(CONVERT(VARCHAR(MAX), REPLACE(hex_Value, 0x00, 0x20))))                              --VARCHAR,CHAR
           WHEN system_type_id = 48 THEN
               CONVERT(VARCHAR(MAX), CONVERT(TINYINT, CONVERT(BINARY(1), REVERSE(hex_Value))))                  --TINY INTEGER
           WHEN system_type_id = 52 THEN
               CONVERT(VARCHAR(MAX), CONVERT(SMALLINT, CONVERT(BINARY(2), REVERSE(hex_Value))))                 --SMALL INTEGER
           WHEN system_type_id = 56 THEN
               CONVERT(VARCHAR(MAX), CONVERT(INT, CONVERT(BINARY(4), REVERSE(hex_Value))))                      -- INTEGER
           WHEN system_type_id = 127 THEN
               CONVERT(VARCHAR(MAX), CONVERT(BIGINT, CONVERT(BINARY(8), REVERSE(hex_Value))))                   -- BIG INTEGER
           WHEN system_type_id = 61 THEN
               CONVERT(VARCHAR(MAX), CONVERT(DATETIME, CONVERT(VARBINARY(8000), REVERSE(hex_Value))), 100)      --DATETIME
           ...
           WHEN system_type_id = 58 THEN
               CONVERT(VARCHAR(MAX), CONVERT(SMALLDATETIME, CONVERT(VARBINARY(8000), REVERSE(hex_Value))), 100) --SMALL DATETIME
           WHEN system_type_id = 108 THEN
               CONVERT(
                          VARCHAR(MAX),
                          CAST(CONVERT(
                                          NUMERIC(38, 30),
                                          CONVERT(VARBINARY, CONVERT(VARBINARY, xprec) + CONVERT(VARBINARY, xscale))
                                          + CONVERT(VARBINARY(1), 0) + hex_Value
                                      ) AS FLOAT)
                      )                                                                                         --- NUMERIC
           WHEN system_type_id IN ( 60, 122 ) THEN
               CONVERT(VARCHAR(MAX), CONVERT(MONEY, CONVERT(VARBINARY(8000), REVERSE(hex_Value))), 2)           --MONEY,SMALLMONEY
           ...
           WHEN system_type_id = 104 THEN
               CONVERT(VARCHAR(MAX), CONVERT(BIT, CONVERT(BINARY(1), hex_Value) % 2))                           -- BIT
           ...
           WHEN system_type_id = 59 THEN
               LEFT(LTRIM(STR(
                                 CAST(SIGN(CAST(CONVERT(VARBINARY(8000), REVERSE(hex_Value)) AS BIGINT))
                                      * (1.0
                                         + (CAST(CONVERT(VARBINARY(8000), REVERSE(hex_Value)) AS BIGINT) & 0x007FFFFF)
           ...
           WHEN system_type_id IN ( 165, 173 ) THEN
       (CASE
            WHEN CHARINDEX(0x, CAST('' AS XML).value('xs:hexBinary(sql:column("hex_Value"))', 'VARBINARY(8000)')) = 0 THEN
                '0x'
            ELSE
                ''
        END
           ...
           WHEN system_type_id = 36 THEN
               CONVERT(VARCHAR(MAX), CONVERT(UNIQUEIDENTIFIER, hex_Value))                                      --UNIQUEIDENTIFIER
       END AS FieldValue,
       [RowLogContents],
       [Transaction ID],
       [Slot ID],
...
WITH CTE
AS (SELECT (CASE
                WHEN system_type_id IN ( 167, 175, 189 ) THEN
                    QUOTENAME([name]) + '=' + ISNULL(+'''' + [A].[FieldValue] + '''', 'NULL') + ' ,' + ' '
                WHEN system_type_id IN ( 231, 239 ) THEN
                    QUOTENAME([name]) + '=' + ISNULL(+'N''' + [A].[FieldValue] + '''', 'NULL') + ' ,' + ''
                WHEN system_type_id IN ( 58, 40, 61, 36 ) THEN
                    QUOTENAME([name]) + '=' + ISNULL(+'''' + [A].[FieldValue] + '''', 'NULL') + '  ,' + ' '
    ...
    FROM #temp_Data AS [A]
        ...
        INNER JOIN sys.columns [D]
            ON [object_id] = OBJECT_ID('' + @SchemaName_n_TableName + '')
    ...
    SELECT (CASE
                ...
                WHEN system_type_id IN ( 231, 239 ) THEN
                    QUOTENAME([name]) + '=' + ISNULL(+'N''' + [A].[FieldValue] + '''', 'NULL') + ' AND ' + ''
                WHEN system_type_id IN ( 58, 40, 61, 36 ) THEN
                    QUOTENAME([name]) + '=' + ISNULL(+'''' + [A].[FieldValue] + '''', 'NULL') + ' AND ' + ''
                WHEN system_type_id IN ( 48, 52, 56, 59, 60, 62, 104, 106, 108, 122, 127 ) THEN
                    QUOTENAME([name]) + '=' + ISNULL([A].[FieldValue], 'NULL') + ' AND ' + ''
    ...
    WHERE ISNULL([A].[FieldValue], '') = ISNULL([B].[FieldValue], '')
     ...
     CTEUpdateQuery
AS (SELECT 'UPDATE ' + @SchemaName_n_TableName + ' SET '
           + LEFT(STUFF(
                  (
                      SELECT ' ' + ISNULL([Field], '') + ' '
                      FROM CTE B
